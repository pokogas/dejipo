    ...
    <v-container fluid>
      ...
      <v-container>
        <v-card>
          <v-simple-table class="mb-10" style="white-space:nowrap;">
            <template>
              ...
              <tr v-if="transaction_data.status === 1">
                <td>ステータス</td>
                <td style="color: rgba(74,197,35,0.7); font-size: 13px" v-if="transaction_data.status ===1">・完了</td>
                <td style="color: rgba(63,63,63,0.72); font-size: 13px" v-if="transaction_data.status ===2">・未支払い</td>
                <td style="color: rgba(63,63,63,0.72); font-size: 13px" v-if="transaction_data.status ===3">・発行済み</td>
              </tr>
              <tr v-if="transaction_data.status === 1">
                <td>ウォレットID</td>
        ...
        <v-row class="justify-end mr-1">
          <v-btn class="mr-2" @click="setData()">更新</v-btn>
          <v-btn class="mr-2" @click="qrModal = true">QR表示</v-btn>
          <v-btn class="mr-2">SNS</v-btn>
        </v-row>
        <v-dialog
          v-if="qrModal"
          v-model="qrModal"
          ...
          <v-card>
            <v-layout justify-center>
              <qrcode :value="transaction_url" :options="{ width: 250 }" class="grey lighten-2"></qrcode>
            </v-layout>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn
